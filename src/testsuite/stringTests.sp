...
public void OnPluginStart() {
  ...
  RegServerCmd("tests.util.string.Array_IntsToString",
      Command_Test_IntsToString);
  RegServerCmd("tests.util.string.Array_FloatsToString",
      Command_Test_FloatsToString);
  RegServerCmd("tests.util.string.String_StringJoin",
...
public Action Command_Test_IntsToString(int args) {
  ...
  char tmp[128];
  int ints1[] = { 2 };
  Array_IntsToString(tmp, sizeof tmp, ints1, sizeof ints1);
  AssertTrue(tmp, StrEqual(tmp, "[2]"));

  int ints2[] = { 2, 3 };
  Array_IntsToString(tmp, sizeof tmp, ints2, sizeof ints2);
  AssertTrue(tmp, StrEqual(tmp, "[2,3]"));

  int ints3[] = { 2, 3, 5, 7, 11 };
  Array_IntsToString(tmp, sizeof tmp, ints3, sizeof ints3);
...
public Action Command_Test_FloatsToString(int args) {
  ...
  char tmp[128];
  float floats1[] = { 2.0 };
  Array_FloatsToString(tmp, sizeof tmp, floats1, sizeof floats1);
  AssertTrue(tmp, StrEqual(tmp, "[2.000000]"));

  float floats2[] = { 2.0, 3.0 };
  Array_FloatsToString(tmp, sizeof tmp, floats2, sizeof floats2);
  AssertTrue(tmp, StrEqual(tmp, "[2.000000,3.000000]"));

  float floats3[] = { 2.0, 3.0, 5.0, 7.0, 11.0 };
  Array_FloatsToString(tmp, sizeof tmp, floats3, sizeof floats3);
  AssertTrue(tmp,
...
public Action Command_Test_StringJoin(int args) {
  ...
  char tmp[128];
  char strings1[][] = { "apple" };
  String_StringJoin(tmp, sizeof tmp, strings1, sizeof strings1);
  AssertTrue(tmp, StrEqual(tmp, "[apple]"));

  char strings2[][] = { "pepperoni", "pizza" };
  String_StringJoin(tmp, sizeof tmp, strings2, sizeof strings2);
  AssertTrue(tmp, StrEqual(tmp, "[pepperoni,pizza]"));

  char strings3[][] = { "this", "is", "a", "test" };
  String_StringJoin(tmp, sizeof tmp, strings3, sizeof strings3);
  AssertTrue(tmp, StrEqual(tmp, "[this,is,a,test]"));

  char strings4[][] = { "models", "player", "gign", "gign.mdl" };
  String_StringJoin(tmp, sizeof tmp, strings4, sizeof strings4,
